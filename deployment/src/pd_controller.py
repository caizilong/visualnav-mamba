...
def pd_controller(waypoint: np.ndarray) -> Tuple[float]:
	"""根据本地坐标系下的 waypoint 计算线速度 v 和角速度 w。"""
	assert len(waypoint) == 2 or len(waypoint) == 4, "waypoint must be a 2D or 4D vector"
	if len(waypoint) == 2:
		dx, dy = waypoint
	else:
		dx, dy, hx, hy = waypoint
	# 若 dx, dy 非常小，则仅依据预测的 heading 来旋转原地对准目标方向
...
def callback_drive(waypoint_msg: Float32MultiArray):
	"""订阅上游导航模块发布的 waypoint，并存入 ROSData。"""
	global vel_msg
